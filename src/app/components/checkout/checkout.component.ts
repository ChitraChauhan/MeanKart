...
export class CheckoutComponent implements OnInit {
  ...
  async payNow(): Promise<void> {
    ...
    try {
      ...
      // Step 1: Create order on our server with cart items
      this.paymentService.createOrder(this.totalAmount, cartItems || []).subscribe({
        next: (response: CreateOrderResponse) => {
          ...
          const options = {
            ...
            items: cartItems,
            handler: (response: RazorpayResponse) => this.handlePaymentResponse(response),
            prefill: {
          ...
          // Close payment window if component is destroyed
  ...
  private async handlePaymentResponse(response: RazorpayResponse): Promise<void> {
    try {
      // Step 1: Verify payment on our server
      ...
      if (verification.success) {
        // Step 2: Clear cart on successful payment
      ...
      this.notificationService.show({
        type: 'error',
        message: 'Error processing your payment. Please check your order status or contact support.',
        duration: 3000
      });
  ...
  private handleSuccessfulPayment(response: RazorpayResponse, verification: PaymentVerificationResponse): void {
    ...
    // Use navigateByUrl with replaceUrl to prevent back navigation to checkout
    this.router.navigateByUrl('/my-orders', {
      state: {
        paymentId: response.razorpay_payment_id,
        orderId: response.razorpay_order_id
      },
      replaceUrl: true  // This replaces the current URL in the history
    }).then(navigated => {
      if (!navigated) {
